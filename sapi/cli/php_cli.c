...
static sapi_module_struct cli_sapi_module = {
	...
	php_cli_startup,				/* startup 函数*/
	php_module_shutdown_wrapper,	/* shutdown 函数*/
...
int main(int argc, char *argv[])
{
	...
	//sapi模块,具体需要查看 main/SAPI.h, 这个变量定义在了本文件: cli/php_cli.c中。
	sapi_module_struct *sapi_module = &cli_sapi_module;

	/**
	 * 解析输入参数。
	 * --------------------------------------------------------------------
	 * 具体做啥还不知道呢。里面一大堆的判断 define。还需要知道各个宏定义的意思
	 */
